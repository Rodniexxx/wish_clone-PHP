...
$total = fetchOne("SELECT COUNT(*) as count FROM news");
$totalPages = ceil($total['count'] / $perPage);

$items = fetchAll("SELECT n.*, c.name as category_name FROM news n LEFT JOIN categories c ON n.category_id = c.id ORDER BY n.created_at DESC LIMIT ? OFFSET ?", [$perPage, $offset]);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>News &amp; Features | Wish 107.5 Admin</title>
...
<body>
    ...
    <div class="main">
        <div class="main-header">
            <h1>News &amp; Features</h1>
            <a href="news-add.php" class="btn"><span class="material-icons">add</span> Add New</a>
        </div>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                ...
                <tr>
                    <td><?= htmlspecialchars(truncate($item['title'], 60)) ?></td>
                    <td><?= htmlspecialchars($item['category_name'] ?? '-') ?></td>
                    <td><?= $item['featured'] ? 'Yes' : 'No' ?></td>
                    <td><?= date('M j, Y', strtotime($item['created_at'])) ?></td>
                    <td><span class="status-badge <?= $item['status'] ?>"><?= $item['status'] ?></span></td>
                    <td class="table-actions">
                        <a href="news-edit.php?id=<?= $item['id'] ?>">Edit</a>
                        <a href="news-edit.php?id=<?= $item['id'] ?>&delete=1" onclick="return confirm('Delete this item?')" style="color:#f44329">Delete</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
