...
if (isset($_GET['delete'])) {
    execute("DELETE FROM shows WHERE id = ?", [$id]);
    header('Location: shows.php');
    exit;
}

$item = fetchOne("SELECT * FROM shows WHERE id = ?", [$id]);
if (!$item) { header('Location: shows.php'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $existing = fetchOne("SELECT id FROM shows WHERE slug = ? AND id != ?", [$slug, $id]);
    $slug = $existing ? $slug . '-' . uniqid() : $slug;

    execute("UPDATE shows SET title=?, slug=?, day_of_week=?, start_time=?, end_time=?, description=?, dj_name=?, image=?, status=? WHERE id=?",
        [$title, $slug, $day_of_week, $start_time, $end_time, $description, $dj_name, $image, $status, $id]);
...
<head>
    ...
    <title>Edit Show | Wish 107.5 Admin</title>
    ...
    <div class="main">
        ...
        <form method="POST" enctype="multipart/form-data" class="form-card">
            ...
            <div class="form-group">
                ...
                <input type="text" id="dj_name" name="dj_name" value="<?= htmlspecialchars($item['dj_name']) ?>">
            </div>
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select id="day_of_week" name="day_of_week" required>
                        ...
                        <option value="Mon-Fri" <?= $item['day_of_week'] == 'Mon-Fri' ? 'selected' : '' ?>>Monday - Friday</option>
                        ...
                        <option value="Daily" <?= $item['day_of_week'] == 'Daily' ? 'selected' : '' ?>>Daily</option>
                    </select>
                </div>
                <div class="form-group">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="start_time" name="start_time" value="<?= htmlspecialchars($item['start_time']) ?>" required>
                </div>
                <div class="form-group">
                    <label for="end_time">End Time</label>
                    <input type="text" id="end_time" name="end_time" value="<?= htmlspecialchars($item['end_time']) ?>" required>
                </div>
            </div>
            <div class="form-group">
