...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $existing = fetchOne("SELECT id FROM videos WHERE slug = ?", [$slug]);
    $slug = $existing ? $slug . '-' . uniqid() : $slug;

    $videoId = insert("INSERT INTO videos (title, slug, artist, description, youtube_id, thumbnail, featured, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        [$title, $slug, $artistName, $description, $youtube_id, $thumbnail, $featured, $status]);

    $artistRecord = fetchOne("SELECT id FROM artists WHERE name = ?", [$artistName]);
    if ($artistRecord) {
        execute("INSERT INTO artist_videos (artist_id, video_id) VALUES (?, ?)", [$artistRecord['id'], $videoId]);
    }
...
<head>
    ...
    <title>Add Wishclusive | Wish 107.5 Admin</title>
